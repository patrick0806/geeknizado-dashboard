...
import { updateProduct, uploadProductImages, findProductBySlug } from '@/services/product';
...
export function EditProductForm({ product, refetch, categories, themes }: {
  product: Product,
  refetch: Function,
              ...
              <div className="flex gap-2 flex-wrap">
                {existingImages.map(img => (
                  <div key={img.id} className="relative group">
                    <img src={img.url} alt="product" className="w-24 h-24 object-cover rounded shadow" />
                    {/* Remove button (optional) */}
                    <button type="button" onClick={() => handleRemoveImage(img.id)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-80 group-hover:opacity-100">&times;</button>
                  </div>
                ))}
                {existingImages.length === 0 && <span className="text-gray-500">Nenhuma imagem</span>}
              </div>
            </div>
            {/* Upload more images */}
            <div>
              <div className="font-semibold mb-2 mt-4">Adicionar novas imagens</div>
